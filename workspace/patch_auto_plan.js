...
const replacement = `shapes.filter(s => s.areaType === 'lawn').forEach(s => {
       ...
       for(let i=0; i<pts.length; i++) {
           ...
           // We sample early to estimate the spacing radius along this edge
           ...
           let currentEdgePos = estRadius * 1.0; // overlap edge-to-edge
           let j = 1;
           let sanity = 0;
       ...
       // Helper to check if a point is covered by a sprinkler
       ...
       // Generate test points for coverage evaluation
       let testPoints: any[] = [];
       ...
       // Backwards removal to prioritize keeping corners (which are at the start of the list)
       ...
       // Clean up diff property before pushing
...
console.log('Patched raycast auto algorithm');
